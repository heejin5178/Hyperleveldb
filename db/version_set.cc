...
static uint64_t MaxCompactionBytesForLevel(unsigned level, const Options* options_) {
  return MaxFileSizeForLevel(level, options_) * 16;
}
  ...
  if (level > 0) {
    ...
    for (size_t i = 0; i < boundaries.size(); ++i) {
      for (size_t j = i; j < boundaries.size(); ++j) {
        ...
        if (sz_a + sz_b >= MaxCompactionBytesForLevel(level, options_)) {
          break;
        }
...
Compaction* VersionSet::CompactRange(
  ...
  // two files overlap.
  if (level > 0) {
    const uint64_t limit = MaxFileSizeForLevel(level, options_);
    uint64_t total = 0;
    for (size_t i = 0; i < inputs.size(); i++) {
