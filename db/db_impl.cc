...
void DBImpl::CompactMemTableThread() {
  ...
  while (!shutting_down_.Acquire_Load()) {
    ...
    assert(options_.level0_slowdown_writes_trigger > 0);
  }
...
void DBImpl::SequenceWriteEnd(Writer* w) {
  ...
  if (w->micros_ > options_.level0_slowdown_writes_trigger) {
    env_->SleepForMicroseconds(w->micros_ - options_.level0_slowdown_writes_trigger);
  }
}
